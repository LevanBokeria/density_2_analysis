...
```{r asymmetric-interaction-skew-rt}

tt_part_sum_stats_correct_ref_triplet_location_easiness %>%
        filter(dep_var_type == 'post_pre_diff',
               triplet_location == 'across_density_regions',
               triplet_easiness == 8) %>%
        droplevels() %>%
        ...
        stat_summary(fun.data = mean_cl_normal,
                     ...
                     width=0.1,
                     color='red') +
        facet_wrap(~triplet_easiness+counterbalancing)
...
```{r triplet-location-differences-easiness-levels, fig.width=8, fig.height=6, warning=F}
