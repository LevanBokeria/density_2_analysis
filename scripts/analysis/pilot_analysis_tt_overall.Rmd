...
source('./scripts/utils/get_diff_scores_between_triplet_locations.R')
...
tt_participant_summary_stats_triplet_location_differences <- tt_participant_summary_stats_triplet_location %>%
...
tt_participant_summary_stats_triplet_location_easiness_differences <- tt_participant_summary_stats_triplet_location_easiness %>%
        pivot_wider(id_cols = c(prolific_id,
                                counterbalancing,
                                triplet_easiness,
                                dep_var_type),
...
# Now plot this
fig1 <- tt_participant_summary_stats_triplet_location_easiness_differences %>%
        filter(dep_var_type == 'post_pre_diff',
               chose_towards_type == 'chose_towards_sparse') %>% 
        ggplot(aes(x=difference_type,
                   y=difference_value)) +
...
# Now plot this
fig2 <- tt_participant_summary_stats_triplet_location_easiness_differences %>%
        filter(dep_var_type == 'post_pre_diff',
               chose_towards_type == 'chose_towards_sparse') %>% 
        ggplot(aes(x=difference_type,
                   y=difference_value)) +
...
tt_long_post_pre_and_diff %>%
        ...
        summarise(n_datapoints = n(),
                  ...
                  mean_correct = mean(correct_avg_across_reps, na.rm = T)) %>%
        ungroup() %>%
        ggplot(aes(x=reorder(triplet_unique_name, -mean_chose_towards_sparse),
                   y=mean_chose_towards_sparse)) +
        geom_boxplot() + 
