...
for iF in file_list:
    ...
    f = open('./data/experiments/gui_downloads/' + iF,'r')    
    ...
    # Get the subject number, not sure if I need this after pilots?
    ...
    # %% Add the counterbalancing and experiment conditions as a columns
    tt.insert(loc=0, column='counterbalancing', value=data_decoded['inputData']['cb_condition'])
    tt.insert(loc=0, column='experiment', value=experiment)
    exp.insert(loc=0, column='counterbalancing', value=data_decoded['inputData']['cb_condition'])        
    exp.insert(loc=0, column='experiment', value=experiment)        
...
# %% Save data files
if save_tt_data:
    large_tt.to_csv('./results/experiments/preprocessed_data/triplet_task_long_form.csv',index=False)
    
if save_debriefing:
    large_debri.to_csv('./results/experiments/preprocessed_data/debriefings.csv',index=False)
    
if save_instructions:
    large_instr.to_csv('./results/experiments/preprocessed_data/instructions.csv',index=False)

if save_breaks:
    large_breaks.to_csv('./results/experiments/preprocessed_data/breaks_feedback.csv',index=False)

if save_exposure:
    large_exp.to_csv('./results/experiments/preprocessed_data/exposure_task_long_form.csv',index=False)