...
manual_qc <- import('./results/pilots/preprocessed_data/qc_manual_for_breaks_debriefing.csv')
...
cb <- tt_long %>% select(prolific_id,pilot_paradigm,counterbalancing) %>% group_by(prolific_id) %>% slice_head()
...
if (plot_tt_per_part){
        for (iP in unique(tt_long$prolific_id)[30:40]){
                print(iP)
...
for (iP in unique(tt_long$prolific_id)[30:31]){
        print(iP)
